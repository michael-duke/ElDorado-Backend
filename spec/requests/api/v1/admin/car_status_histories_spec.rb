  ...
  path '/api/v1/admin/cars/{car_id}/status_histories' do
    ...
    get 'Retrieve Car Maintenance History (Admin Only)' do
      ...
      response '200', 'Logs retrieved successfully' do
        ...
        let(:admin) { User.create!(role: 1, email: 'admin_audit@test.com', password: 'password', name: 'Admin Michael') }
        let(:car) { Car.create!(name: 'Camry', status: 'available', model: '2024', daily_price: '100', image: 'i.png', description: 'Toyota Camry desc') }
        let(:car_id) { car.id }
      ...
      response '404', 'Car not found' do
        ...
        let(:car_id) { 99999 } # Non-existent ID
