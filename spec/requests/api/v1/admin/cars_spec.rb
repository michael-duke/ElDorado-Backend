  ...
  path '/api/v1/admin/cars' do
    get 'List all Cars (Admin Only)' do
      ...
      response '200', 'All cars retrieved' do
        ...
        before do
          Car.create!(name: 'Tesla', image: 'tesla.png', model: 'Model S', daily_price: 150, description: 'Electric',
                      status: 'available')
          sign_in @admin
        end
        run_test!
  ...
  path '/api/v1/admin/cars/{id}' do
    ...
    patch 'Update a Car' do
      ...
      response '200', 'Car updated successfully' do
        schema '$ref' => '#/components/schemas/car_single_response'
        let(:existing_car) do
          Car.create!(name: 'Old Mustang', image: 'mustang.png', model: '2020', daily_price: 50,
                      description: 'Old American muscle', status: 'reserved')
        end
  ...
  path '/api/v1/admin/cars/{id}/availability' do
    ...
    patch 'Toggle Car Availability' do
      ...
      response '200', 'Status toggled successfully' do
        schema '$ref' => '#/components/schemas/car_single_response'
        let(:existing_car) do
          Car.create!(name: 'Toggle', image: 'i.png', model: '2020', daily_price: 50, description: 'A toggle desc',
                      status: 'available')
        end
        let(:id) { existing_car.id }
      ...
      response '409', 'Conflict - Car is currently reserved' do
        ...
        let(:id) { car.id }
        let(:car) do
          Car.create!(name: 'Tesla', status: 'reserved', model: 'Model Y', daily_price: '200.44', image: 'i.png',
                      description: 'Tesla Model Y desc')
        end
